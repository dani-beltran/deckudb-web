...
const SENSITIVE_TEXT_PATTERN =
  /((["']?)\b(?:authorization|cookie|credential|password|passwd|(?:[a-z][a-z0-9_-]*)?(?:secret|token)|session(?:[-_]?id)?|api[-_]?key|user(?:[-_]?(?:id|identity|name))?|email(?:[-_]?address)?|(?:client[-_]?)?ip(?:[-_]?address)?|remote[-_]?address)\b\2\s*[:=]\s*)(?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|(?:basic|bearer)\s+[^\s,;]+|[^\s,;]+)/gi
