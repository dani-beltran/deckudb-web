...
const SENSITIVE_TEXT_PATTERN =
  /((["']?)\b(?:authorization|cookie|credential|password|passwd|secret|session(?:[-_]?id)?|token|api[-_]?key|user(?:[-_]?(?:id|identity|name))?|email(?:[-_]?address)?|(?:client[-_]?)?ip(?:[-_]?address)?|remote[-_]?address)\b\2\s*[:=]\s*)(?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|(?:basic|bearer)\s+[^\s,;]+|[^\s,;]+)/gi
